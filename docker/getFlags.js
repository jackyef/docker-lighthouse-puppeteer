...
function getFlags(manualArgv) {
  ...
  const argv = y
    ...
    .example(
      "lighthouse <url> --view",
      "Opens the HTML report in a browser after the run completes"
    )
    .example(
    ...
    .example(
      'lighthouse <url> --add-browser-cookie="foo:bar"',
      "Add the cookie to the browser instance before running lighthouse"
    )

    .example(
      'lighthouse <url> --browser-cookie-domain=".example.com"',
      "The domain the cookies will be set to. (Only used when using --add-browser-cookie"
    )
    /**
    ...
    .describe({
      "cli-flags-path":
        "The path to a JSON file that contains the desired CLI flags to apply. Flags specified at the command line will still override the file-based ones.",
      ...
      "emulated-form-factor":
        "Controls the emulated device form factor (mobile vs. desktop) if not disabled",
      "throttling-method": "Controls throttling method",
      "throttling.rttMs": "Controls simulated network RTT (TCP layer)",
      "throttling.throughputKbps":
        "Controls simulated network download throughput",
      "throttling.requestLatencyMs":
        "Controls emulated network RTT (HTTP layer)",
      "throttling.downloadThroughputKbps":
        "Controls emulated network download throughput",
      "throttling.uploadThroughputKbps":
        "Controls emulated network upload throughput",
      "throttling.cpuSlowdownMultiplier":
        "Controls simulated + emulated CPU throttling",
      "gather-mode":
      ...
      "audit-mode":
        "Process saved artifacts from disk. (Artifacts folder path may be provided, otherwise defaults to ./latest-run/)",
      ...
      "precomputed-lantern-data-path":
        "Path to the file where lantern simulation data should be read from, overwriting the lantern observed estimates for RTT and server latency.",
      "lantern-data-output-path":
        "Path to the file where lantern simulation data should be written to, can be used in a future run with the `precomputed-lantern-data-path` flag.",
    ...
    .describe({
      output: `Reporter for the results, supports multiple values. choices: ${printer
        .getValidOutputOptions()
        .map(s => `"${s}"`)
        .join(", ")}`,
  ...
  Example: --output-path=./lighthouse-results.html`,
    ...
    .check(
      /** @param {LH.CliFlags} argv */ argv => {
        ...
        const isPrintSomethingMode =
          argv.listAllAudits || argv.listTraceCategories || argv.printConfig;
